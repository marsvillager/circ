...
pub struct NonMembershipProverWitness {
	pub input_domain_name_wildcard: Vec<u8>,
	pub root: String,
...
impl Witness for NonMembershipProverWitness {
	fn to_map(&self) -> WitnessMapper {
		...
		mapper.map_u8_arr_padded(&self.input_domain_name_wildcard, 255, "input_domain_name_wildcard");
...
pub struct NonMembershipVerifierWitness {
	pub input_domain_name_wildcard: Vec<String>,
	pub root: String,
	pub ret: String
}
...
impl Witness for NonMembershipVerifierWitness {
	fn to_map(&self) -> WitnessMapper {
		let mut mapper = WitnessMapper::new();
		mapper.map_field_arr_padded(&self.input_domain_name_wildcard, 255, "input_domain_name_wildcard");
		mapper.map_field(&self.root, "root");
